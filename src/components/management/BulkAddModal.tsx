                        ...
                        <thead>
                            <tr>
                                ...
                                {columns.map((col) => (
                                    <th
                                        key={col.key}
                                        className={cn("text-left text-[9px] font-bold uppercase tracking-widest text-muted-foreground pb-3 pr-3", col.width)}
                                    >
                                        {col.label}
                                    </th>
                                ))}
                        ...
                        <tbody className="space-y-2">
                            {rows.map((row, rowIndex) => {
                                ...
                                return (
                                    <tr
                                        key={rowIndex}
                                        className={cn("group transition-colors rounded-xl", isFailed ? "bg-red-50" : "hover:bg-muted/20")}
                                    >
                                        <td className="py-2 pr-3 text-[11px] font-bold text-muted-foreground align-middle">
                                            {isFailed ? <AlertCircle className="w-4 h-4 text-destructive" /> : <span className="opacity-40">{rowIndex + 1}</span>}
                                        </td>
                                        {columns.map((col) => (
                                            <td
                                                key={col.key}
                                            ...
                                            >
                                                {col.options ? (
                                                    ...
                                                    <Input
                                                        ...
                                                        className={cn("h-10 rounded-xl text-xs", isFailed ? "border-destructive focus-visible:ring-destructive/20" : "")}
                                                    />
                                                )}
                                            </td>
                                        ))}
