...
import {
  Loader2, Plus, Pencil, Trash2, Save, X, ArrowRight,
...
function DpBadge({ status, amount }: { status?: string; amount?: string }) {
  ...
  return (
    <div className="flex flex-col items-end gap-1">
      ...
      <Badge variant="outline" className={cn('rounded-full text-[9px] font-black px-2 py-0 border-0 uppercase leading-none scale-90 origin-right', cfg)}>
        {label}
      </Badge>
    </div>
  );
    ...
    <div className="flex flex-col items-end gap-1">
      ...
      {isRefunded ? (
        <div className="flex flex-col items-end gap-0.5">
          <Badge variant="outline" className="rounded-full text-[9px] font-black px-2 py-0 border-0 bg-emerald-50 text-emerald-700 leading-none scale-90 origin-right uppercase">
            Refunded
          </Badge>
          {refundedAt && (
            <span className="text-[9px] text-muted-foreground scale-90 origin-right leading-none mt-0.5">
              {format(parseISO(refundedAt), 'dd/MM/yyyy')}
            </span>
          )}
        </div>
      ) : (
        <Badge variant="outline" className="rounded-full text-[9px] font-black px-2 py-0 border-0 bg-blue-50 text-blue-700 leading-none scale-90 origin-right uppercase">
          Held
        </Badge>
      )}
    </div>
...
export default function RentalManagement({
  ...
  return (
    <Card className="bg-white/70 backdrop-blur-xl border-white/20 shadow-soft rounded-[2rem] overflow-hidden">
            ...
            <TableBody>
              {rentals.length === 0 ? (
                ...
                rentals.map((rental) => {
                  ...
                  const currentRentAmount = rental.Monthly_Rent && rental.Monthly_Rent !== '0'
                    ? parseInt(rental.Monthly_Rent)
                    : parseInt(room?.Harga_Sewa || '0');
                    ...
                    >
                      ...
                      <TableCell>
                        <div className="flex flex-col gap-1 items-start text-left">
                          ...
                          <Badge variant="outline" className="rounded-full text-[9px] font-black uppercase tracking-wider px-2 py-0 border-0 bg-muted text-muted-foreground mt-0.5 leading-none">
                            {trajectory.duration}
                          </Badge>
                        </div>
                      </TableCell>
                      ...
                      <TableCell className="text-right">
                        <div className="flex flex-col gap-1.5 items-end">
                          ...
                          <DpBadge status={rental.DP_Status} amount={rental.DP_Amount} />
                        </div>
                      </TableCell>
                      ...
                      <TableCell className="text-right pr-8">
                        <div className="flex justify-end gap-2">
                          <Button size="icon" variant="ghost" onClick={() => onEdit(rental)} className="h-10 w-10 rounded-xl text-muted-foreground hover:text-primary hover:bg-primary/10 transition-all cursor-pointer">
                            <Pencil className="w-4 h-4" />
                          </Button>
                          <Button size="icon" variant="ghost" disabled={actionLoading === `delete-${rental.ID_Sewa}`} onClick={() => onDelete(rental.ID_Sewa)} className="h-10 w-10 rounded-xl text-muted-foreground hover:text-destructive hover:bg-destructive/10 transition-all cursor-pointer">
      ...
      <Dialog open={isFormOpen} onOpenChange={(open) => { if (!open) onCancel(); }}>
        <DialogContent className="max-w-4xl w-full bg-white rounded-[2rem] p-0 overflow-hidden shadow-2xl border-0 gap-0 font-sans">
          ...
          <DialogHeader className="p-8 bg-muted/20 border-b border-border text-left relative overflow-hidden shrink-0">
            ...
            <DialogDescription className="text-muted-foreground text-xs font-semibold uppercase tracking-widest mt-1">
              {editingId === 'new' ? 'Tambah Sewa/Booking Baru' : 'Edit Kontrak Transaksi'}
            </DialogDescription>
          </DialogHeader>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-2 text-left">
                      ...
                      <div className="flex gap-2">
                        ...
                        />
                        <Select value={editFormData.Unit_Durasi || 'Bulan'} onValueChange={set('Unit_Durasi')}>
                          <SelectTrigger className="h-12 rounded-xl flex-1"><SelectValue /></SelectTrigger>
                          <SelectContent>
                            {DURASI_OPTIONS.map(opt => <SelectItem key={opt.value} value={opt.value}>{opt.label}</SelectItem>)}
                          </SelectContent>
                        </Select>
                      </div>
                    </div>
                ...
                <div className="p-6 bg-muted/20 border border-border rounded-2xl space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="space-y-2 text-left">
                      ...
                      <Input type="date" value={editFormData.Tgl_DP || ''} onChange={e => set('Tgl_DP')(e.target.value)} className="h-12 rounded-xl" />
                    </div>
                    <div className="space-y-2 text-left">
                    ...
                    <div className="col-span-2 space-y-2 text-left">
                      ...
                      <Select value={editFormData.DP_Status || 'pending'} onValueChange={set('DP_Status')}>
