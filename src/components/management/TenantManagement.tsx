...
export default function TenantManagement({ tenants, editingId, editFormData, isAdding, actionLoading, selectedIds, onEdit, onSave, onDelete, onCancel, onStartAdding, onBulkAdd, onToggleSelect, setEditFormData }: TenantManagementProps) {
    const allSelected = tenants.length > 0 && tenants.every((t) => selectedIds.includes(t.ID_Penghuni));

    const toggleAll = () => {
        if (allSelected) {
            tenants.forEach((t) => {
                if (selectedIds.includes(t.ID_Penghuni)) onToggleSelect(t.ID_Penghuni);
            });
        } else {
            tenants.forEach((t) => {
                if (!selectedIds.includes(t.ID_Penghuni)) onToggleSelect(t.ID_Penghuni);
                        ...
                        <TableBody>
                            {isAdding && editingId === "new" && (
                                <TableRow className="border-border bg-orange-50/30">
                                    ...
                                    <TableCell>
                                        <Input
                                            value={editFormData.Kontak_Darurat || ""}
                                            onChange={(e) => setEditFormData({ ...editFormData, Kontak_Darurat: e.target.value })}
                                            className="h-10 rounded-xl"
                                            placeholder="628xxxx"
                                        />
                                    </TableCell>
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <TableCell className="font-medium text-muted-foreground">
                                            {isEditing ? (
                                                <select
                                                    value={editFormData.Bawa_Mobil || ""}
                                                    onChange={(e) => setEditFormData({ ...editFormData, Bawa_Mobil: e.target.value })}
                                                    className="bg-white border border-border text-foreground p-1 rounded-lg h-9 text-xs font-bold"
                                                >
                                                    <option value="Tidak">Tidak</option>
                                                    <option value="Ya">Ya</option>
                                                </select>
                                                ...
                                                <Badge
                                                    variant={tenant.Bawa_Mobil === "Ya" ? "default" : "outline"}
                                                    className={cn(
                                                        "rounded-full text-[10px] font-bold px-3",
                                                        tenant.Bawa_Mobil === "Ya"
                                                            ? "bg-gradient-to-r from-slate-300 via-white to-slate-300 text-slate-700 border border-slate-300/80 shadow-md shadow-slate-500/20 animate-shimmer"
                                                ...
                                                {isEditing ? (
                                                    ...
                                                    <>
                                                        ...
                                                        <Button
                                                            size="icon"
                                                            variant="ghost"
                                                            disabled={actionLoading === `delete-${tenant.ID_Penghuni}`}
                                                            onClick={() => onDelete(tenant.ID_Penghuni)}
