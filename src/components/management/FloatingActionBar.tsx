...
export default function FloatingActionBar({ selectedCount, actions, onClearSelection, loading = false }: FloatingActionBarProps) {
            ...
            <AnimatePresence>
                {selectedCount > 0 && (
                    ...
                    >
                        ...
                        {/* Count badge */}
                        <Badge className="bg-primary text-white border-0 font-bold rounded-xl px-3 shrink-0 shadow-lg shadow-orange-500/20">{selectedCount} dipilih</Badge>
                        ...
                        <div className="flex items-center gap-2">
                            {actions.map((action, idx) => {
                                ...
                                return (
                                    ...
                                    >
                                        {loading ? <Loader2 className="w-3.5 h-3.5 animate-spin mr-1.5" /> : Icon ? <Icon className="w-3.5 h-3.5 mr-1.5" /> : null}
                                        {action.label}
                                    </Button>
                                );
                            })}
...
export function makeStatusAction(label: string, status: string, selectedCount: number, onExecute: () => Promise<boolean>): FabAction {
    ...
    return {
        ...
        onExecute,
        className: isActive ? "bg-emerald-500 hover:bg-emerald-600 text-white h-9 px-4 rounded-xl font-bold text-xs" : "bg-rose-500 hover:bg-rose-600 text-white h-9 px-4 rounded-xl font-bold text-xs",
    };
}
