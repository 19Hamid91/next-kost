...
export default function TenantViewMode({ room, tenant, rental, isBooked, rentalStatus, getPeriodeLabel, waReminderUrl }: TenantViewModeProps) {
    if (isBooked) {
        return (
            <>
                <section className="space-y-4">
                    ...
                    <div className="grid gap-3">
                        ...
                        <InfoRow
                            icon={Car}
                            label="Kendaraan"
                            value={tenant?.Bawa_Mobil === "Ya" ? "Mobil" : "—"}
                        />
                        {tenant?.Kontak_Darurat && (
                            <InfoRow
                                icon={Phone}
                                label="Kontak Darurat"
                                value={tenant.Kontak_Darurat}
                ...
                <section className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        ...
                        <ContractCard
                            label="Tgl. DP"
                            value={rental?.Tgl_DP ? format(parseISO(rental.Tgl_DP), "d MMM yyyy", { locale: localeId }) : "—"}
                        />
                        <ContractCard
                            label="Periode"
                            value={getPeriodeLabel(rental?.Periode_Sewa || "1", rental?.Unit_Durasi)}
                        />
                        <ContractCard
                            label="Deposit"
                            value={`Rp ${parseInt(rental?.Nominal_Deposit || "0").toLocaleString("id-ID")}`}
                        />
                    </div>
                </section>
            </>
