...
const FLOOR_BG_CLASSES = [
  'bg-purple-100/50 border-purple-200/60',
  'bg-sky-100/50 border-sky-200/60',
...
function RoomNumber({ number, align }: { number: string | number; align: 'left' | 'right' }) {
  return (
    <div className={`flex items-center ${align === 'left' ? 'justify-end' : 'justify-start'}`}>
      <span className="text-[11px] font-black text-foreground/40 tabular-nums select-none">
        {number}
      </span>
    </div>
  );
...
function FloorSection({
  ...
  const sortedRooms = [...roomsWithOccupancy].sort(
    (roomA, roomB) => Number(roomA.room.No_Kamar) - Number(roomB.room.No_Kamar)
  );
      ...
      >
        ...
        {pairs.map(([leftOccupancy, rightOccupancy], pairRowIndex) => (
          <Fragment key={`${tabId}-pair-${pairRowIndex}`}>
            {/* Left room number */}
            <div style={{ gridColumn: 1, gridRow: pairRowIndex + 1 }} className="flex items-center justify-center">
              <RoomNumber number={leftOccupancy.room.No_Kamar} align="right" />
            </div>
            ...
            {/* Right room number */}
            <div style={{ gridColumn: 5, gridRow: pairRowIndex + 1 }} className="flex items-center justify-center">
              {rightOccupancy && <RoomNumber number={rightOccupancy.room.No_Kamar} align="left" />}
            </div>
          </Fragment>
        ))}
      </div>
