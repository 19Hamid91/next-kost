...
export default function RoomCard({ room, tenant, rental, nextRental, nextTenant, upcomingCount = 0, compact = false, onClick }: RoomCardProps) {
    ...
    const nextMoveIn = nextRental?.Tgl_Masuk ? format(parseISO(nextRental.Tgl_Masuk), "MMM yy", { locale: localeId }) : null;
        ...
        >
            ...
            {upcomingCount > 0 && (
                <div className="absolute top-2 right-2 flex items-center gap-1 bg-amber-100/90 border border-amber-200/60 px-1.5 py-0.5 rounded-full select-none shadow-[0_1px_2px_rgba(0,0,0,0.02)] z-10">
                    ...
                    <span className="text-[8px] font-black tracking-wider text-amber-800 uppercase leading-none">+{upcomingCount} Booking</span>
                </div>
            )}
            ...
            <div className="flex-1 p-3 flex flex-col gap-1">
                ...
                {!compact && <span className={cn("text-2xl font-extrabold tracking-tight leading-none", numberColor)}>{room.No_Kamar}</span>}

                {/* Tenant name or vacant icon */}
                {isOccupied && tenant ? (
                    <span className={cn("text-[10px] font-black uppercase tracking-wide leading-tight truncate", labelColor)}>{tenant.Nama}</span>
                ) : !isOccupied ? (
