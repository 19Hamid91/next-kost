  ...
  return (
    <div className="space-y-6">
            ...
            >
              {isLoading ? (
                ...
                <div className="bg-white/70 backdrop-blur-xl rounded-[1.5rem] p-6 border border-border shadow-soft hover:shadow-md transition-shadow">
                  ...
                  <p className={`text-xl font-extrabold tracking-tight ${card.colorClass}`}>
      ...
      <motion.div
        ...
        className={`rounded-[1.5rem] px-8 py-5 flex items-center justify-between border ${
          isPositive
      ...
      >
        <div>
          ...
          <p className={`text-2xl font-extrabold tracking-tight mt-1 ${isPositive ? 'text-emerald-600' : 'text-destructive'}`}>
            {isLoading ? (
              <span className="inline-block h-8 w-40 bg-muted animate-pulse rounded" />
            ) : (
              `${isPositive ? '+' : ''}${formatRupiah(netCashflow)}`
            )}
          </p>
        </div>
        {!isLoading && (
          <div className={`text-xs font-bold px-4 py-2 rounded-xl ${
            isPositive ? 'bg-emerald-500/15 text-emerald-700' : 'bg-destructive/15 text-destructive'
          }`}>
