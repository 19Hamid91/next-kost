        ...
        <Card className="bg-white/70 backdrop-blur-xl border-border shadow-soft rounded-[2rem] overflow-hidden">
            <CardHeader className="flex flex-row items-center justify-between border-b border-border p-6">
                <div className="space-y-1">
                    ...
                    <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-[0.18em]">Operational costs this month</p>
                </div>
                {!showForm && (
                    <Button
                        size="sm"
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                            ...
                            <div className="flex flex-col gap-1.5">
                                ...
                                <Input
                                    ...
                                    onFocus={(e) => {
                                        if (e.target.value === "0") setForm((prev) => ({ ...prev, amount: "" }));
                                    }}
                                    placeholder="150000"
                                    className="h-10 rounded-xl"
                                />
                        ...
                        <TableHeader className="bg-muted/30">
                            <TableRow className="border-border">
                                {["Date", "Category", "Amount", "Notes", ""].map((head, idx) => (
                                    <TableHead
                                        key={idx}
                                        className={cn("text-[10px] font-bold uppercase tracking-widest text-muted-foreground py-4", idx === 4 ? "text-right pr-8 w-16" : "")}
                                    >
                                        {head}
                                    </TableHead>
                                ))}
                        ...
                        <TableBody>
                            {expenses.length === 0 ? (
                                ...
                                expenses.map((expense) => (
                                    ...
                                    >
                                        <TableCell className="font-medium text-muted-foreground py-4">{formatDate(expense.Date)}</TableCell>
                                        <TableCell>
                                            <span className="text-[10px] font-bold uppercase tracking-widest px-3 py-1 bg-muted rounded-full text-muted-foreground">{categoryLabel(expense.Category)}</span>
                                        </TableCell>
                                        <TableCell className="font-bold text-destructive text-right">{formatRupiah(expense.Amount)}</TableCell>
                                        <TableCell className="text-muted-foreground text-sm max-w-[200px] truncate">{expense.Notes || "—"}</TableCell>
                                        <TableCell className="text-right pr-8">
                                            ...
                                            >
                                                {actionLoading === `delete-${expense.ID_Expense}` ? <Loader2 className="w-4 h-4 animate-spin" /> : <Trash2 className="w-4 h-4" />}
                                            </Button>
                                        </TableCell>
                                    </TableRow>
                                ))
