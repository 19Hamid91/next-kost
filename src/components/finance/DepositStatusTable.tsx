...
const formatRupiah = (amount: string | number) => `Rp ${parseInt(String(amount) || "0").toLocaleString("id-ID")}`;
...
function MarkReturnedButton({ bookingId, isLoading, onConfirm }: { bookingId: string; isLoading: boolean; onConfirm: () => void }) {
        ...
        <div className="flex justify-end items-center gap-1.5 min-w-[120px]">
            <AnimatePresence mode="wait">
                {!pending ? (
                    ...
                    >
                        <span className="text-[10px] font-semibold text-muted-foreground mr-1 whitespace-nowrap">Confirm?</span>
                        <Button
                            size="icon"
...
export default function DepositStatusTable({ depositRows, allRooms, allTenants, actionLoading, onRefund, sectionRef }: DepositStatusTableProps) {
        ...
        >
            <CardHeader className="border-b border-border p-6">
                <div className="space-y-1">
                    ...
                    <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-[0.18em]">Active tenants + last 3 months</p>
                </div>
            </CardHeader>
                        ...
                        <TableHeader className="bg-muted/30">
                            <TableRow className="border-border">
                                {["Tenant", "Room", "Check-in", "Check-out", "Deposit", "Status", "Action"].map((head, idx) => (
                                    <TableHead
                                        key={head}
                                        className={cn("text-[10px] font-bold uppercase tracking-widest text-muted-foreground py-4", idx === 6 ? "text-right pr-8" : "")}
                                    >
                                        {head}
                                    </TableHead>
                                ))}
                            ...
                            {filteredRows.length === 0 ? (
                                ...
                                filteredRows.map((rental) => {
                                    ...
                                    return (
                                        <TableRow
                                            key={rental.ID_Sewa}
                                            className={cn("border-border hover:bg-muted/20 transition-colors", isRefunded ? "opacity-60" : "")}
                                        >
                                            <TableCell className="font-bold text-foreground py-4">{tenant?.Nama ?? rental.ID_Penghuni}</TableCell>
                                            <TableCell className="font-medium text-muted-foreground">{room?.No_Kamar ?? rental.ID_Kamar}</TableCell>
                                            <TableCell className="text-muted-foreground text-sm">{formatDate(rental.Tgl_Masuk)}</TableCell>
                                            <TableCell className="text-muted-foreground text-sm">{endDateStr}</TableCell>
                                            <TableCell className="font-semibold text-foreground text-right">{formatRupiah(rental.Nominal_Deposit ?? 0)}</TableCell>
                                            <TableCell>
                                                {isRefunded ? (
                                                    <Badge
                                                        variant="outline"
                                            ...
                                            <TableCell className="text-right pr-8">
                                                {!isRefunded ? (
                                                    ...
                                                    <span className="text-[10px] text-muted-foreground">{rental.Deposit_Refunded_At ? formatDate(rental.Deposit_Refunded_At) : "—"}</span>
                                                )}
                                            </TableCell>
                                        </TableRow>
                                    );
