...
export default function ManagementPage() {
    ...
    const fabActions = (() => {
        if (activeTab === "rentals") {
            return [
                makeStatusAction("→ AKTIF", "AKTIF", selectedIds.length, () => handleBulkUpdate("Transaksi_Sewa", "ID_Sewa", { Status_Sewa: "AKTIF" })),
                makeStatusAction("→ SELESAI", "SELESAI", selectedIds.length, () => handleBulkUpdate("Transaksi_Sewa", "ID_Sewa", { Status_Sewa: "SELESAI" })),
            ];
        }
        return [];
    })();
                ...
                >
                    <div className="flex justify-center w-full">
                        <TabsList className="bg-white/50 backdrop-blur-md p-1.5 md:p-2 md:px-4 rounded-[1.25rem] md:rounded-[1.5rem] h-14 md:h-16 border border-border shadow-soft w-full flex">
                            <TabsTrigger
                                value="rooms"
                                className="flex-1 rounded-xl h-full text-[10px] md:text-[11px] font-bold uppercase tracking-widest data-[state=active]:bg-white data-[state=active]:text-primary data-[state=active]:shadow-sm transition-all whitespace-nowrap"
                            >
                                Kamar
                            </TabsTrigger>
                            <TabsTrigger
